...
void pack(unsigned int *packed, InstData *data) {
    *packed = 0;
    
    // Empacotamento para cada tipo de instrucao
    if (data->type == R) {
        *packed |= (data->opcode);
        *packed |= (data->rd << 7);
        *packed |= (data->funct3 << 12);
        *packed |= (data->rs1 << 15);
        *packed |= (data->rs2 << 20);
        *packed |= (data->funct7 << 25);
    } else if (data->type == I) {
        *packed |= (data->opcode);
        *packed |= (data->rd << 7);
        *packed |= (data->funct3 << 12);
        *packed |= (data->rs1 << 15);
        *packed |= ((data->imm & 0xFFF) << 20); // 12 bits do imediato
    } else if (data->type == S) {
        *packed |= (data->opcode);
        *packed |= ((data->imm & 0x1F) << 7); // 5 bits do imediato
        *packed |= (data->funct3 << 12);
        *packed |= (data->rs1 << 15);
        *packed |= (data->rs2 << 20);
        *packed |= ((data->imm >> 5) << 25); // 7 bits do imediato
    } else if (data->type == B) {
        *packed |= (data->opcode);
        *packed |= ((data->imm & 0x1E) >> 1 << 8); // imm[4:1]
        *packed |= ((data->imm & 0x800) >> 11 << 7); // imm[11]
        *packed |= (data->funct3 << 12);
        *packed |= (data->rs1 << 15);
        *packed |= (data->rs2 << 20);
        *packed |= ((data->imm & 0x7E0) >> 5 << 25); // imm[10:5]
        *packed |= ((data->imm & 0x1000) >> 12 << 31); // imm[12]
    } else if (data->type == U) {
        *packed |= (data->opcode);
        *packed |= (data->rd << 7);
        *packed |= ((data->imm & 0xFFFFF) << 12); // 20 bits do imediato
    } else {
        *packed |= (data->opcode);
        *packed |= (data->rd << 7);
        *packed |= ((data->imm & 0x7FE) >> 1 << 21); // imm[10:1]
        *packed |= ((data->imm & 0x800) >> 11 << 20); // imm[11]
        *packed |= ((data->imm & 0xFF000) >> 12 << 12); // imm[19:12]
        *packed |= ((data->imm & 0x100000) >> 20 << 31); // imm[20]
    }
}
...
int main() {
    ...
    n = read(STDIN_FD, (void*)buffer, 40);

    InstData data; // iniciando struct
